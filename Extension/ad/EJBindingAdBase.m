...
- (void)triggerEventOnce:(NSString *)type {
	[self triggerEventOnce:type argc:0 argv:nil];
}
...
//		NSLog(@"key : %@, value: %@", key, value);
		...
		if ( isFunc ){;
			NSString *callbackType = [[type stringByAppendingString:@"_"] stringByAppendingString:key];
			[self setCallbackWithType:callbackType ctx:ctx callback:jsValue];
		}else {
